// Usage: Place me at <src>/contrib/libtests/driver_rw.c
// To work with AFL, use CMakeLists.txt.driver_rw.afl
...
static int
buffer_from_array(struct buffer *buffer, const uint8_t *Data, size_t Size) 
{
   ...
   for (;;)
   {
      ...
      if (Size > 0) {
         r = Size;
         if (r > (sizeof last->buffer)-count) {
            r = (sizeof last->buffer)-count;
         }
         memcpy(last->buffer+count, Data, r);
         Size -= r;
         Data = Data + r;
      }
...
static void
display_cache_array(struct display *dp, const uint8_t* Data, size_t Size)
   /* Does the initial cache of the array. */
{
   dp->filename = NULL;
   buffer_from_array(&dp->original_file, Data, Size);
}
...
static void
test_one_file(struct display *dp, const uint8_t* Data, size_t Size)
{
   ...
   dp->transforms = 0;
   display_cache_array(dp, Data, Size);
   update_display(dp);
...
static int
do_test(struct display *dp, const uint8_t* Data, size_t Size)
